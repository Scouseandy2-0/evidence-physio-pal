-- Fix array issue and populate assessment tools database
...
-- Update CPD activities table structure for proper column names
UPDATE cpd_activities 
SET hours_earned = hours_claimed, 
    completion_date = date_completed
WHERE hours_earned IS NULL AND completion_date IS NULL;

-- Create sample study groups if none exist
INSERT INTO study_groups (name, description, topic, focus_area, max_members, is_public)
SELECT 
